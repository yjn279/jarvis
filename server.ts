...
const mcp = new Server(
  { name: 'jarvis', version: '1.0.0' },
  {
    ...
    instructions: [
      ...
      '',
      'Messages from Discord arrive as <channel source="jarvis" chat_id="..." message_id="..." user="..." ts="...">. If the tag has attachment_count, the attachments attribute lists name/type/size — call download_attachment(chat_id, message_id) to fetch them. Reply with the reply tool — pass chat_id back. A mention in a channel auto-opens a thread and chat_id points at it, so your reply lands in the thread; keep replying with that chat_id to stay in the thread. Use reply_to (set to a message_id) only when replying to an earlier message; the latest message doesn\'t need a quote-reply, omit reply_to for normal responses.',
      '',
...
// jarvis: derive a thread title from the opening message. Strip the bot
// mention, collapse whitespace, cap well under Discord's 100-char limit.
function threadName(content: string): string {
  const t = (content || '').replace(/<@!?\d+>/g, '').replace(/\s+/g, ' ').trim()
  return t.slice(0, 80) || '会話'
}
